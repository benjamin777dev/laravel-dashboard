...
window.updateDataDeal = function (dealId, dbDealId) {
    ...
    // return;
    if (client_name_primary.length <= 0 && window.selectedGroupsDefault <= 0) {
...
window.updateDealData = async function (field, id, dealID, value = null) {
    ...
    let formData = {
        data: [
            {
                Deal_Name:
                    field == "deal_name" ? elementId?.textContent : undefined,
                Client_Name_Primary:
                ...
                Sale_Price:
                    field == "sale_price" ? elementId?.textContent : undefined,
                Closing_Date:
                    field == "closing_date"
                        ? elementId?.textContent
                            ? elementId?.textContent
                        ...
                        : undefined,
                Commission:
                    field == "commission" ? elementId?.textContent : undefined,
