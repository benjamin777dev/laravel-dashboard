                        ...
                        <div class="col-sm-8">
                            <div class="text-sm-end">
                                <button type="button" data-bs-toggle="modal" data-bs-target="#newContactModal"
                                    class="btn btn-dark btn-rounded waves-effect waves-light addContact-modal mb-2"><i
                                        class="mdi mdi-plus me-1"></i> New Contact</button>
                                        <button type="button" data-bs-toggle="modal" data-bs-target="#newContactModal"
                                    class="btn btn-dark btn-rounded waves-effect waves-light addContact-modal mb-2"><i
                                        class="mdi mdi-plus me-1"></i> New Transaction</button>
                            </div>
                        </div>
                        ...
                        <table class="table align-middle table-nowrap table-hover dt-responsive nowrap w-100"
                            ...
                            <tbody>
                                @foreach ($contacts as $contact)
                                    <tr>
                                        <td><a href="{{ route('contacts.show', $contact['id']) }}"><img src="{{ URL::asset('/images/open.svg') }}" alt="Open icon"
                                        class="ppiplinecommonIcon" title="Contact Details"></a>{{ $contact['first_name'] . ' ' . $contact['last_name'] ?? 'N/A' }}</td>
                                        <td>{{ $contact['abcd'] ?? '-' }}</td>
                                        ...
                                        <td>
                                            @php
                                                $address = '';
                                                if (!empty($contact['mailing_address'])) $address .= $contact['mailing_address'];
                                                if (!empty($contact['mailing_city'])) $address .= ($address ? ', ' : '') . $contact['mailing_city'];
                                                if (!empty($contact['mailing_state'])) $address .= ($address ? ', ' : '') . $contact['mailing_state'];
                                                if (!empty($contact['mailing_zip'])) $address .= ($address ? ', ' : '') . $contact['mailing_zip'];
                                                $contact['address'] = $address ?: 'N/A';
                                            @endphp
