                        ...
                        <div class="npcontactsBody">
                                ...
                                <div class="commonFlex pipelinestatusdiv">
                                    <select class="form-select pstatusText"
                                        ...
                                        @foreach ($allstages as $stage)
                                            <option value="{{ $stage }}"
                                                {{ $deal['stage'] == $stage ? 'selected' : '' }}>{{ $stage }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="" style="width: 75px;">
                                <select class="form-select npinputinfo" id="representing{{ $deal['zoho_deal_id'] }}"
                                    ...
                                    <option value="Buyer" {{ $deal['representing'] == 'Buyer' ? 'selected' : '' }}>Buyer
                                    </option>
                            ...
                            <div class="commonTextEllipsis"
                                ...
                                {{ number_format($deal['sale_price'] ?? '0', 0, '.', ',') }}</div>
                            <div onclick="updateDeal('{{ $deal['zoho_deal_id'] }}','closing_date','{{ $deal['id'] }}')"
                                id="closing_date{{ $deal['zoho_deal_id'] }}">
                                {{ \Carbon\Carbon::parse($deal['closing_date'])->format('Y-m-d') ?? 'N/A' }}</div>
                            <div>
                                <div class="commonTextEllipsis"
                                    onclick="updateDeal('{{ $deal['zoho_deal_id'] }}','commission','{{ $deal['id'] }}')"
                                    id="commission{{ $deal['zoho_deal_id'] }}">
                                    {{ number_format($deal['commission'] ?? '0', 2) }}%</div>
                            </div>
                            <div>
                                <div class="commonTextEllipsis">
                ...
                success: function(data) {
                    ...
                    if (isMobile) {
                        if (data.length === 0) {
                            ...
                            ptableCardDiv.append(
                                '<div class="pnofound"><p class="text-center">No records found</p></div>');
                            return;
                        }
                    } else {
                        if (data.length === 0) {
                            // If no data found, display a message
                            ppipelineTableBody.append(
                                '<div class="pnofound"><p class="text-center">No records found</p></div>');
                            return;
                        }
                    }
